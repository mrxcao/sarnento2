                    ...
                    <table className="table table-hover">
                        <thead>
                            <tr>
                                ...
                                <th className="border-gray-200">Mensagem</th>
                        ...
                        <tbody>
                            {logs.map(log => (
                                <tr key={log._id}>
                                    <td>{new Date(log.criado).toLocaleString()}</td>
                                    <td>{log.guild ? log.guild.name : log.idGuild} <br/><small className="text-muted">{log.idGuild}</small></td>
                                    <td>{log.user ? (log.user.nm_user || log.user.username) : log.idUSr} <br/><small className="text-muted">{log.idUSr}</small></td>
                                    <td style={{whiteSpace: 'pre-wrap', maxHeight: '100px', overflowY: 'auto', display: 'block'}}>{log.msg}</td>
