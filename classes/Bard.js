...
class ChatBot {
	...
	async getAPI() {
		...
		return new Promise((resolve) => {
			https.get(this.homeurl, optionss, (responseFromServer) => {
				let responseContent = '';
				if (responseFromServer.statusCode != 200) {resolve(`Status Code: ${responseFromServer.statusCode}`);}
				responseFromServer.on('data', (chunk) => {
					responseContent += chunk;
				});

				responseFromServer.on('end', () => {
					const SNlM0e = this.searchInText(responseContent, /"SNlM0e":"([^"]*)"/);
					const cfb2h = this.searchInText(responseContent, /"cfb2h":"([^"]*)"/);
					this.snlm0e = SNlM0e;
					this.cfb2h = cfb2h;
					resolve({
						snlm0e: SNlM0e,
	...
	async ask(prompt) {
		...
		const at = encodeURIComponent(this.snlm0e);
		...
		const uri = this.chaturl + this.cfb2h;

		return new Promise((resolve) => {
			const request = https.request(uri, optionss, (response) => {
				let responseText = '';
				if (response.statusCode != 200) {console.log('Server returned status code ' + response.statusCode);}
				...
				response.on('end', () => {
					...
					try {
						...
						console.log('e', e);
						resolve('dog fora de serviço. mals ae');
					}
				});
			});
